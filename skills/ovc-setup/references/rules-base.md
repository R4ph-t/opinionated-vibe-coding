...
Do not hand-roll solutions for problems that established libraries solve. This includes but is not limited to: date/time manipulation, HTTP clients, validation, authentication, CSV/JSON/XML parsing, encryption/hashing, email sending, image processing.
...
- Set secure defaults for CORS, rate limiting, HTTPS, and headers.
...
- If you notice something wrong with the existing code, flag it even if it's not part of the current task.
